...
class smart_wait():
    def __init__(self, num_batches:int=10, wait_secs:float=10.0):
...
class classification_model():
    def __init__(self, excel_dir:str, num_batches:int=10, wait_secs:float=10.0):
    ...
    def translate_sheet(self):
        ...
        row_fixer = 0
        for i, sentence in enumerate(self.sheet['P'][1:] + self.sheet['Q'][1:]):
            ...
            original_col = 'A' if sentence.column == 16 else 'C'
            translation_col = 'B' if sentence.column == 16 else 'D'

            self.translation_sheet[f"{original_col}{sentence.row - row_fixer}"].value = text
